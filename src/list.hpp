...
template <class T>
list<T>::const_iterator::const_iterator(const iterator& other) : cur{other.cur}{}

template <class T>
list<T>::const_iterator::const_iterator(node *ptr): cur{ptr} {}

template <class T>
list<T>::const_iterator::const_iterator(const const_iterator& other): cur{other.cur} {}
...
template <class T>
typename list<T>::const_iterator list<T>::insert(const_iterator pos, const T& value)
{
    size_l++;
...
template <class T>
typename list<T>::const_iterator list<T>::insert(const_iterator pos, size_t count, const T &value) {
    if (count == 0)
        return const_iterator(pos.cur);
...
typename list<T>::const_iterator list<T>::erase(const_iterator pos) {
    ...
    if (pos == const_iterator(last_ptr))
...
void list<T>::unique()
{
    ...
    while (cur != last_ptr)
    {
        if (cur->data == cur->next->data)
        {
            ...
            std::cout << cur->data << "--" << new_cur->next->data << std::endl;
            auto it_beg = const_iterator(cur);
            auto it_end = const_iterator(new_cur->next);
